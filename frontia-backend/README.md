...
Frontend 0.7 ยังใช้ save protocol 2 แต่ต้องมี `features.storyboard: 1` จาก Backend 0.7 หาก Backend ยังเป็นรุ่น 0.6 แอปจะเก็บข้อมูลในเครื่องและแจ้งให้อัปเดต ไม่ส่งช็อต/สถานะใหม่ไปให้เซิร์ฟเวอร์ที่อาจตัดข้อมูลเหล่านี้ทิ้ง และไม่เรียก AI รุ่นเก่าที่ตอบเป็นฉากข้อความเดียว ส่วน client เก่าที่ไม่มี `expectedRevision` ได้รับ HTTP 428
...
- ส่ง 12 ฉากล่าสุด สถานะ เวลา เบาะแส ความสัมพันธ์ และสรุปความทรงจำสะสมให้ AI ประวัติเต็มยังอยู่ในเซฟ ไม่รับประกันว่าโมเดลจะจำทุกเหตุการณ์ถูกต้อง
- สร้างครั้งละหนึ่งคำขอให้ตอบเป็น 4–7 ช็อตพร้อมตัวละคร สีหน้า ฉากหลัง ผลของฉาก และทางเลือก รูปแบบผ่าน JSON schema และ validation; ภาพมาจากชุดที่มีในแอปเท่านั้น ไม่มี URL ภาพจากโมเดล
- จำผลคำขอที่สำเร็จแล้ว 10 นาทีในหน่วยความจำเพื่อกัน request ID ซ้ำ การ restart ล้าง cache นี้
- `/health` ตรวจ HTTP server ไม่ใช่หลักฐานว่าล็อกอิน Codex แล้วหรือโควตายังเหลือ

```bash
curl https://YOUR-BACKEND/health
# {"ok":true,"protocol":2,"features":{"storyboard":1},"version":"0.7.0"}
```
