...
## สิ่งที่แก้ในการตรวจรอบนี้

- ฉากเปิด ตัวละคร ประวัติ ความทรงจำ และเซฟแยกแต่ละเรื่อง แก้ทุกเรื่องใช้ฉากเดียวกัน
- แสดงจำนวนฉากจริงแทนเปอร์เซ็นต์ที่คำนวณขึ้นเอง แชตเป็นทางลัดเข้าบทสนทนาในเรื่อง บันทึก/สถานที่/สถานะ/เมนูใช้งานได้
...
- PWA มีไอคอนและ manifest เก็บเฉพาะ app shell ไม่ cache API หรือข้ามไปลบ cache ของโปรเจกต์อื่นบน origin เดียวกัน
...
| `ui.mjs`, `styles.css` | หน้าตาและ responsive layout |
...
ข้อมูลต้นฉบับ v4/v5 ใน localStorage ไม่ถูกลบ เซฟใหม่แปลงเฉพาะฉาก/ประวัติที่มีอยู่จริง เวอร์ชันเดิมเก็บฉากล่าสุดเพียงชุดเดียว จึงกู้ฉากที่ไม่เคยเก็บแยกเรื่องไม่ได้ ควร export สำรองก่อนอัปเดต Backend และหลีกเลี่ยงเล่นพร้อมกันหลายแท็บ
