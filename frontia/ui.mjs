...
export function renderView(ctx) {
  ...
  if (view === 'home') return page(`
    ...
    ${started.length ? `<h2>อ่านต่อจากที่ค้างไว้</h2>${started.map(key => storyRow(key, stories[key])).join('')}` : ''}
  ...
  if (view === 'player' && ep && stories[id] && !stories[id].deleted) {
    ...
    return `<main class="player cinematic-player" id="main" tabindex="-1">${visual}
      <section class="reader-controls">${notices}<div class="tools">${button('journal', '▤ บันทึก', 'tool')}${button('locations', '⌘ สถานที่', 'tool')}${button('story-status', '◎ สถานะ', 'tool')}</div><div class="storybody"><div class="reader-meta"><span>ช่วงที่ ${story.history.length} · ${esc(scene.location)}</span>${saveStatus}</div>
